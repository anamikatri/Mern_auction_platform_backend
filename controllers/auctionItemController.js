    ...
    try {
        const cloudinaryResponse = await cloudinary.uploader.upload(image.tempFilePath,{
          ...
          return res.status(201).json({
            success: true,
            message: `Auction item creadted and will be listed on auction page at ${startTime}`,
            auctionitem,
          });
    } catch (error) {
...
export const republishItems=catchAsyncErrors(async(req,res,next)=>{
    ...
    if (!req.body.startTime || !req.body.endTime) {
        return next(new ErrorHandler("Starttime and endtime for republish is manadatory."));
    }
    ...
    res.status(200).json({
        success:true,
        auctionItem,
        message:`Auction republished and willbe active on ${req.body.startTime}`,
    })
});
